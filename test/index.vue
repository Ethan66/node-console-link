      ...
      >
        ...
        >
          <div v-for="(item, index) in configs" :key="item.id">
            <el-row><h3 style="font-size: 18px; margin-top: 0;">{{ index + 1 }}、{{ item.title }}</h3></el-row>
            <p v-for="child in item.list" :key="child.field" style="display: inline-block; margin: 0;">
              <el-form-item :label="child.label" :prop="child.field">
                ...
                <el-select
                  ...
                  v-model="form[child.field]"
                  size="small"
                  @change="(a, b) => {onChangeSwitch($event, index)}"
                >
                  <el-option
                    v-for="(node, i) in optDeductSwitch"
                    :key="i"
                    :label="node.label"
                    :value="node.value"
                  />
  ...
  export default {
    ...
    methods: {
      ...
      handleGetConfigs(spreadType) {
        ...
        if (spreadType === 3) { // 半流程
          result.push({ title: '借款金额', prefix: 'borrowAmount', list: Object.values(ob).map(item => ({ ...item, field: 'borrowAmount' + item.type })) })
        } else if ([1, 4].includes(spreadType)) { // 导流、泛舟
          result = [
            { title: '注册人数', prefix: 'register', list: Object.values(ob).filter(item => item.label !== '长尾计算').map(item => ({ ...item, field: 'register' + item.type })) },
            { title: '授信人数（一推授信）', prefix: 'credit', list: Object.values(ob).map(item => ({ ...item, field: 'credit' + item.type })) },
            { title: '借款人数', prefix: 'borrow', list: Object.values(ob).map(item => ({ ...item, field: 'borrow' + item.type })) }
          ]
        }
        return result
      },
      ...
      handleGetFormRules(configs, formData) {
        ...
        configs.forEach(item => {
          ...
          if (ob) { required = !!formData[ob.field] }
          item.list.reduce((data, cur) => {
      ...
      onChangeSwitch(val, index) {
        this.configs[index].list.map(item => item.field).forEach(key => { this.rules[key][0].required = !!val })
      },
      onClose() {
      ...
      onSubmit() {
        this.$refs.dialogDataForm.validate(valid => {
          if (valid) {
            this.isLoading = true
            apiAddOrUpdateDeduction({ ...this.form }).then(res => {
